...
public:
  ...
  // Does not generate nulls when n=0.
  ...
  SimpleVectorPtr<EvalType<T>> encodedVector(
	  ...
	  const std::vector<std::optional<T>> &data) {
	switch (vecType) {
	case VectorEncoding::Simple::FLAT:return flatVectorNullable(data);
	case VectorEncoding::Simple::CONSTANT:return constantVector(data);
	case VectorEncoding::Simple::DICTIONARY:return dictionaryVector(data);
	case VectorEncoding::Simple::SEQUENCE:return sequenceVector(data);
	case VectorEncoding::Simple::BIASED:return biasVector(data);
	default: VELOX_UNSUPPORTED("Unsupported encoding type for VectorMaker.");
	}
	return nullptr;
  }
