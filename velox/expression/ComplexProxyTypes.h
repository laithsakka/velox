...
class ArrayProxy {
 ...
 public:
  ...
  FOLLY_ALWAYS_INLINE element_t& add_item() {
    return add_item_internal<true>();
  }

  // Same as add_item but assumes that array have enough capacity.
  FOLLY_ALWAYS_INLINE element_t& add_item_unchecked_cap() {
    return add_item_internal<false>();
  }

  // Add a new null item to the array.
  FOLLY_ALWAYS_INLINE void add_null() {
    add_null_internal<true>();
  }

  // Same as add_null but assumes that array have enough capacity.
  FOLLY_ALWAYS_INLINE void add_null_unchecked_cap() {
    add_null_internal<false>();
  }
 ...
 private:
  ...
  template <bool doReserve>
  FOLLY_ALWAYS_INLINE element_t& add_item_internal() {
    commitMostRecentChildItem();
    auto index = valuesOffset_ + length_;
    if constexpr (doReserve) {
      reserve(index + 1);
    }
    length_++;
  ...
  template <bool doReserve>
  FOLLY_ALWAYS_INLINE void add_null_internal() {
    commitMostRecentChildItem();
    if constexpr (doReserve) {
